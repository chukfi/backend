package usercache

// in memory user cache implementation

import (
	"sync"
	"time"

	"github.com/chukfi/backend/database/schema"
)

type UserCache struct {
	cache map[string]schema.User
	mu    sync.RWMutex
	ttl   time.Duration
}

var UserCacheInstance *UserCache

func initUserCache(ttl time.Duration) {
	UserCacheInstance = &UserCache{
		cache: make(map[string]schema.User),
		ttl:   ttl,
	}
}

func (uc *UserCache) Get(userID string) (schema.User, bool) {
	uc.mu.RLock()
	defer uc.mu.RUnlock()
	user, exists := uc.cache[userID]
	return user, exists
}

func (uc *UserCache) Set(userID string, user schema.User) {
	uc.mu.Lock()
	defer uc.mu.Unlock()
	uc.cache[userID] = user
	// start a goroutine to delete the user after ttl
	go func() {
		time.Sleep(uc.ttl)
		uc.mu.Lock()
		defer uc.mu.Unlock()
		delete(uc.cache, userID)
	}()
}

func (uc *UserCache) Delete(userID string) {
	uc.mu.Lock()
	defer uc.mu.Unlock()
	delete(uc.cache, userID)
}

func (uc *UserCache) Clear() {
	uc.mu.Lock()
	defer uc.mu.Unlock()
	uc.cache = make(map[string]schema.User)
}

func (uc *UserCache) Size() int {
	uc.mu.RLock()
	defer uc.mu.RUnlock()
	return len(uc.cache)
}

func (uc *UserCache) GetAllUsers() []schema.User {
	uc.mu.RLock()
	defer uc.mu.RUnlock()
	users := make([]schema.User, 0, len(uc.cache))
	for _, user := range uc.cache {
		users = append(users, user)
	}
	return users
}

func init() {
	initUserCache(5 * time.Minute)
}
